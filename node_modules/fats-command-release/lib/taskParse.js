...
_.options = function (options) {
	...
	for (var optionsKey in options) {
		if ('undefined' != typeof options[optionsKey]) {
			var parsed = _[optionsKey](options[optionsKey]);
			_parsed[optionsKey] = parsed;
		} else {
			_parsed[optionsKey] = taskOptions[optionsKey];
		}
	}
	return _parsed;
};
 ...
 * 解析 options
 * */
_.optionsLint = function (options) {
	var _parsed = {};
	for (var optionsKey in options) {
		if ('undefined' != typeof options[optionsKey]) {
			var parsed = _[optionsKey](options[optionsKey]);
...
_.filesPara = function (strPara, root, parseRegex) {
	var taskConfig = fats.taskConfig, arrPara = [], data = [], flag = false;
 ...
 * 返回正则匹配的文件及夹列表
 * */
_.regexDirs = function (regex, dirs) {
	var result = [];
	...
	for (var index in dirs) {
		...
		if (fats.util.isArray(regex)) {
			for (var i = 0; i < regex.length; i++) {
				var str = regex[i]+'$';
				var _temp = dir.match(str);
				if (_temp) {
					result.push(_temp.input);
				}
			}
		} else {
			var _temp = dir.match(regex+'$');
			if (_temp) {
				result.push(_temp.input);
			}
		}
 ...
 * 解析任务 file 参数
 * */
_.paseFilePara = function (taskPara, root) {
	return {
		dest: _.filesPara(taskPara.dest, root, false),
		src: _.filesPara(taskPara.src, root, true)
	};
};

...
exports.register = function (commander) {
	...
	var templateOpenTag = fats.config.get('templateOpenTag');
	var templateCloseTag = fats.config.get('templateCloseTag');
	if (!welcomePage) {
	...
	commander
		.option('-w, --watch [watch]', 'automatic refresh browsers', Boolean, false)
		...
		.action(function () {
			...
			var watch = options.watch == true ? 'on' : 'off';
			...
			var daemon = require('daemonize2').setup({
				...
				argv: [
					...
					'--templateOpenTag', templateOpenTag,
					'--templateCloseTag', templateCloseTag,
					'--root', root,
					'--port', port,
					'--watch', watch,
					'--hostname', hostname
			...
			var restartEvent = function () {
				...
				daemon = require('daemonize2').setup({
					...
					argv: [
						...
						'--templateOpenTag', templateOpenTag,
						'--templateCloseTag', templateCloseTag,
						'--root', root || serverCfg.root,
						'--port', port || serverCfg.port,
						'--watch', watch || serverCfg.watch,
						'--hostname', hostname || serverCfg.hostname
					]
				});

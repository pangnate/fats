...
<script type="text/javascript" src="http://{webSocketServer}/socket.io/socket.io.js"></script>
<script type="text/javascript">
	(function () {
			...
			socket.on('response', function (response) {
				...
				switch (action) {
					case 'reload':
						...
						href = href.replace(/(&?)_fats\=\d{0,}/gi, '').replace(/(&?)_fats_st\=\d{0,}/gi, '');

						if (/\?/i.test(href)) {
							href = href.replace(/\?/i, '?_fats=' + t + "&_fats_st=" + st);
						} else if (/\#/.test(href)) {
							href = href.replace(/\#/i, '?_fats=' + t + "&_fats_st=" + st + '#');
						} else {
							href += '?_fats=' + t + "&_fats_st=" + st;
						}
		...
		// scrollTop
		setTimeout(function(){
			var st = getQueryString('_fats_st');

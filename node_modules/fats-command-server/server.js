...
var server = http.createServer(function (req, res) {
	...
	var root = argv.root,
		reqUrl = req.url,
		ext = path.extname(reqUrl).replace(/^\./, '').replace(/\?.*/, ''),
		RegExpMock = /^\/mock\/.*?/i,
	 ...
	 * mock api
	 * */
	var output = function (content, delay) {
		if (!delay) {
			delay = 200;
		}
		setTimeout(function () {
	...
	if (RegExpMock.test(reqUrl)) {
		...
		// conf.json
		var confFile = root + '/.mock/conf.json';
		if (!fats.util.exists(confFile)) {
			sendErrorPage();
			return;
		}

		var conf = fats.util.readJSON(confFile);
		...
		// file exists
		var fileContent = fats.util.read(filePath),
			code = fileContent.replace(/\/\*[\W\w]*?\*\//g, '')
				.replace(/\/\/.*/g, ''),
			isModule = /exports\s{0,}\=[\W\w]*\}/i,
			isScript = /function[\W\w]*return/i,
			sleep = code.match(/fats\.sleep\((\d+)\)/) || [0, 200];

		if (isModule.test(fileContent)) {  // like module
			delete require.cache[filePath];
			try {
				var content = require(filePath)(req, res, fats);
				output(content, sleep[1]);
			} catch (e) {
				sendErrorPage(500);
			}
		} else if (isScript.test(fileContent)) {  // like script
			try {
				var content = eval(fileContent);
				output(content, sleep[1]);
			} catch (e) {
				sendErrorPage(500);
			}
		} else {
			output(fileContent, sleep[1]);
		}


	} else if (/(less|styl)/i.test(ext)) { // LESS STYLUS
		var pathname = url.parse(req.url).pathname;
		var realPath = path.join(argv.root, path.normalize(pathname.replace(/\.{2,}/g, '')));
		var parser = require('./parser/' + ext);
		var source = fats.util.read(realPath);
		parser(source, function (err, content) {
			if (err) {
					...
					if (err) {
						...
						if (fats.util.inArray(ext, ['htm', 'html', 'shtml'])) {
							...
							try {
								...
								file = html.toString().replace(/\{timestamp\}/ig, t);
							} catch (err) {
								res.end(err);
							}

							// pre replace less styl
							file = file.replace(/rel="stylesheet\/(.*?)"/ig, 'rel="stylesheet"');
						...
						if ('on' == argv.watch && fats.util.inArray(ext, ['html', 'htm', 'shtml']) && req.headers['x-requested-with'] != 'XMLHttpRequest') {
							...
							append = append.toString().replace(/{webSocketServer}/g, host + ':' + argv.port);
							file += '\n' + append;
						}
...
var walker = ndir.walk(argv.root);
var dirList = [argv.root];
	...
	if ('on' == argv.watch) {
		...
		io.sockets.on('connection', function (socket) {
			for (var i = 0; i < dirList.length; i++) {
				watchEvent(dirList[i], io.sockets);
			}
		});
	}
});
